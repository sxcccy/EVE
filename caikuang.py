...
def test():
    ...
    for ix in range(x1, x2):
        for iy in range(y1, y2):
            ...
            colorHex = hex(rgba[0])[2:].zfill(
                2) + hex(rgba[1])[2:].zfill(2) + hex(rgba[2])[2:].zfill(2)
            fill = PatternFill(
                fill_type='solid',
                start_color=colorHex,
                end_color=colorHex)
            ws.cell(row=iy - y1 + 1, column=ix - x1 + 1).fill = fill
            ws.cell(row=iy - y1 + 1, column=ix - x1 + 1).value = str(ix) + \
                ',' + str(iy) + '=' + str(r) + ',' + str(g) + ',' + str(b)
